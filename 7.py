...
def show_high_score(x,y):
    highscore = font.render("Highscore : " + str(score_value), True, (255,255,255))
    screen.blit(highscore,(x+800,y))
def game_over_text():
...
def wasCollision(obstacleX, obstacleY, bulletX, bulletY):
    distance1 = math.sqrt((math.pow(obstacleX-bulletX,2)) + (math.pow(obstacleY - bulletY,2)))
    if distance1 < 25:
        return True
    else:
        return False
    ...
    if bullet_state == "fire":
        ...
        if collision:
            try:
                ...
                game_over_text()
                h = highscore
                if h >= highscore:                #Real highscore to br displayed in the end.
                    highscore = h
                else :
                    highscore = highscore
